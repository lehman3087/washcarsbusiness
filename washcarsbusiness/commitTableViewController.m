...
{
    //根据页面类型不同需要配置的部分
    _iPs_PageName=@"所有产品评论列表"; //页面名称(用于标记页面参数配置)
    ...
    _iPs_POSTAction =@"search_comment_goods"; //销售记录
...
-(void)loginCompletion:(NSNotification*)notification {
    ...
    if ([lsLogin isEqual:@"0"]) {
        ...
        //切换用户
        NSLog(@"切换用户");
        _iPs_POSTID = [theData objectForKey:@"ID"];
        [self startRequest];
    }
...
- (void)addFooter
{
    __unsafe_unretained commitTableViewController *vc = self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //使用自定义的单元格
    commitTableCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier  forIndexPath:indexPath];
    ...
    @try {
        ...
        lsDate =[dict objectForKey:@"goods_date"];
        
        lsprice =[dict objectForKey:@"shop_price"];
        
        lscommitrank =[dict objectForKey:@"goods_com_rank"];
        lscount =[dict objectForKey:@"count"];
        lsnotread =[dict objectForKey:@"notreadcount"];
        
        if ([lscount isEqualToString:@"0"]) {
            lscommitrank = @"3.0";
            cell.txtScore.textColor = [UIColor lightGrayColor];
        }
        ...
        cell.txtPrice.text = [NSString stringWithFormat:@"%@元", lsprice];
        cell.txtDate.text =lsDate;
        cell.txtScore.text =lscommitrank;
        //cell.txtDate.text =lscount;
        [cell initstarRating];
        [cell setRating:[lscommitrank floatValue]];
        
        cell.txtMemo.text = [NSString stringWithFormat:@"%@条评价    %@条未读", lscount , lsnotread];
        
        cell.bgScore.layer.cornerRadius = 6;
        cell.bgScore.layer.masksToBounds = YES;
        cell.bgScore.layer.borderColor = [UIColor lightGrayColor].CGColor;
        cell.bgScore.layer.borderWidth = 1;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    {
        commitListViewController *itemdetail = segue.destinationViewController;
        ...
        NSLog(@"进入明细页面 %d %@", selectedRow ,strName);
...
-(void)reloadView:(NSDictionary*)res
{
    @try{
        ...
        if ([resultCodeObj integerValue] ==0){
            //解析订单明细：
            NSDictionary* code_info = [res objectForKey:@"comment_goods"];
