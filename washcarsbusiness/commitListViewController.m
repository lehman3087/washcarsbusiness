...
- (void)initPage
{
    ...
    _iPs_POST=@"act=%@&business_id=%@&goods_id=%@";
    
    _iPs_POST2=@"act=%@&business_id=%@&goods_id=%@&page=%d"; //请求数据POST参数模板
    ...
    _iPs_POST3=@"act=%@&business_id=%@&goods_id=%@";
...
- (void)clearunread{
    if (_unUnread>0) {
        ...
        _countnoread = [[NSString alloc]initWithFormat:@"未读评论(%d)", _unUnread ];
        [_btnShowunRead setTitle:_countnoread forState:UIControlStateNormal];
    }else{
        return;
    }
...
-(void)startRequest
{
    ...
    if ([_iPs_POSTQueryOption isEqualToString:@"0"]) {
        //评价汇总
        post = [NSString stringWithFormat:_iPs_POST,_iPs_POSTAction, _iPs_POSTID, _itemid];
        ...
        post = [NSString stringWithFormat:_iPs_POST2,_iPs_POSTAction2, _iPs_POSTID, _itemid,_iPageIndex ];
        ...
        if ([_iPs_POSTQueryRegion isEqualToString:@"0"]) {
            ...
            post = [post stringByAppendingString:@"&not_read=1"];
        }
        
    }else if([_iPs_POSTQueryOption isEqualToString:@"2"]){
        //未读评价标记
        post = [NSString stringWithFormat:_iPs_POST3,_iPs_POSTAction3, _iPs_POSTID, _itemid ];
    ...
    @try{
        ...
        if ([resultCodeObj integerValue] ==0){
            ...
            if ([_iPs_POSTQueryOption isEqualToString:@"0"]) {
                ...
                int resultunread = [[res objectForKey:@"unread_count"]intValue];
                ...
                _countnoread = [[NSString alloc]initWithFormat:@"未读评论(%d)", resultunread ];//resultunread
