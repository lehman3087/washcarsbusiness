//
//  salesDetailListViewController.m
//  washcarsbusiness
//
...
#import "MJRefresh.h"
#import "salesDetailViewCell.h"

@interface salesDetailListViewController (){
...
@implementation salesDetailListViewController
...
-(void) viewWillAppear:(BOOL)animated{
    ...
    //NSString *strName = [[NSString alloc] initWithFormat:@"%@ ( %@ )",_itemname,_itemdate];
    NSString *strName = [[NSString alloc] initWithFormat:@"消费明细 %@ ",_itemdate];
    _txtName.text = _itemname; //strName;
...
{
    //根据页面类型不同需要配置的部分
    _iPs_PageName=@"每天产品消费详情"; //页面名称(用于标记页面参数配置)
    //_iPs_URL=@"http://114.112.73.223:8080/"; //请求数据接口模板--地址
    washcarsAppDelegate *delegate=(washcarsAppDelegate*)[[UIApplication sharedApplication]delegate];
    _iPs_URL=delegate.iPs_URL; //请求数据接口模板--地址
    
    _iPs_PAGE=@"business_app.php"; //请求数据接口模板--页面
    _iPs_POST=@"act=%@&business_id=%@&goods_id=%@&use_date=%@"; //请求数据POST参数模板
    
    _iPs_POSTID= delegate.userid ; //请求数据POST参数ID1
    
    _iPs_POSTAction =@"search_code_list"; //消费的消费码记录
...
- (void)addFooter
{
    __unsafe_unretained salesDetailListViewController *vc = self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *CellIdentifier = @"daycodecell";
    
    //    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier  forIndexPath:indexPath];
    //使用自定义的单元格
    salesDetailViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier  forIndexPath:indexPath];
    
    if (cell == nil) {
        NSLog(@"表格初始化错误");
        NSArray * nib = [[NSBundle mainBundle] loadNibNamed:@"daycodecell" owner:self options:nil] ;
    ...
    @try {
        ...
        lsDate =[dict objectForKey:@"use_time"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"daycodecell" forIndexPath:indexPath];
...
-(void)startRequest
{
    ...
    NSString *post = [NSString stringWithFormat:_iPs_POST,_iPs_POSTAction,_iPs_POSTID, _itemid , _itemdate ];
...
-(void)reloadView:(NSDictionary*)res
{
    @try{
        ...
        if ([resultCodeObj integerValue] ==0){
            //解析订单明细：
            NSDictionary* code_info = [res objectForKey:@"code_list"];
