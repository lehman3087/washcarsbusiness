...
#import "MJRefresh.h"
#import "salesDayViewCell.h"
#import "salesDetailListViewController.h"

@interface salesDayListViewController (){
...
@implementation salesDayListViewController
...
-(void) viewWillAppear:(BOOL)animated{
    ...
    _txtName.text = _itemname;
    _txtDate.text = _itemdate;
    _txtconsume.text = _itemconsume;
    _txtSaled.text = _itemcount;
...
{
    //根据页面类型不同需要配置的部分
    _iPs_PageName=@"产品的消费列表"; //页面名称(用于标记页面参数配置)
    //_iPs_URL=@"http://114.112.73.223:8080/"; //请求数据接口模板--地址
    washcarsAppDelegate *delegate=(washcarsAppDelegate*)[[UIApplication sharedApplication]delegate];
    _iPs_URL=delegate.iPs_URL; //请求数据接口模板--地址
    
    _iPs_PAGE=@"business_app.php"; //请求数据接口模板--页面
    _iPs_POST=@"act=%@&business_id=%@&goods_id=%@"; //请求数据POST参数模板
    
    _iPs_POSTID= delegate.userid ; //请求数据POST参数ID1
    
    _iPs_POSTAction =@"search_day_list"; //消费记录
...
- (void)addFooter
{
    __unsafe_unretained salesDayListViewController *vc = self;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *CellIdentifier = @"daysalecell";
    
    //    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier  forIndexPath:indexPath];
    //使用自定义的单元格
    salesDayViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier  forIndexPath:indexPath];
    
    if (cell == nil) {
        NSLog(@"表格初始化错误");
        NSArray * nib = [[NSBundle mainBundle] loadNibNamed:@"daysalecell" owner:self options:nil] ;
    ...
    @try {
        ...
        lsDate =[dict objectForKey:@"use_date"];

        lscount =[dict objectForKey:@"code_count"];
        
        cell.txtNumber.text = [NSString stringWithFormat:@"%d", indexPath.row + 1];
        
        cell.txtDate.text =lsDate;
        cell.txtCount.text = lscount;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    // Get the new view controller using [segue destinationViewController].
    // Pass the selected object to the new view controller.
    if([segue.identifier isEqualToString:@"daycodelist"])
    {
        ...
        NSString *strusedate = [dict objectForKey:@"use_date"];
        NSString *strcount = [dict objectForKey:@"code_count"];
...
//
        itemdetail.itemid = _itemid;
        itemdetail.itemname = _itemname;
        itemdetail.itemconsume = strcount;//_itemconsume;
        
        itemdetail.itemdate = strusedate;
        
        NSLog(@"进入消费明细 %d %@", selectedRow ,strusedate);
...
-(void)reloadView:(NSDictionary*)res
{
    @try{
        ...
        if ([resultCodeObj integerValue] ==0){
            //解析订单明细：
            NSDictionary* code_info = [res objectForKey:@"day_codes_list"];
